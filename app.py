...
def jugadores():
    ...
    if seleccion:
        # Obtener los jugadores del país seleccionado
        cursor.execute("SELECT id, nombre, apellido FROM jugadores WHERE nacionalidad = %s", (seleccion,))
        jugadores = cursor.fetchall()

    if jugador_id:
        # Obtener toda la información del jugador seleccionado
        cursor.execute("""
            SELECT nombre, apellido, edad, equipo_actual, historial_equipos, 
                   tarjetas_rojas, tarjetas_amarillas, imagen, altura, peso, 
                   goles_totales, asistencias_totales, partidos_totales, 
                   lesiones_recientes, redes_sociales, pie_dominante, fecha_nacimiento 
